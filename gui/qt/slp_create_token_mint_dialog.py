...
from electrum_zclassic.util import bfh, format_satoshis_nofloat, format_satoshis_plain_nofloat, NotEnoughFunds, ExcessiveFee, PrintError #, finalization_print_error
from electrum_zclassic.transaction import Transaction
from electrum_zclassic.slp import SlpMessage, SlpNoMintingBatonFound, SlpUnsupportedSlpTokenType, SlpInvalidOutputMessage, buildMintOpReturnOutput_V1
...
class SlpCreateTokenMintDialog(QDialog, MessageBoxMixin, PrintError):
    ...
    def mint_token(self, preview=False):
        ...
        try:
            ...
            self.show_message(_("Enter a Mint Receiver Address in ZSLP address format."))
            return
        ...
        if not self.token_fixed_supply_cb.isChecked():
            try:
                ...
                self.show_message(_("Enter a Baton Address in ZSLP address format."))
                return
